...
namespace UniversityJournal.Core.UseCases
{
    public class DeleteSubjectUseCase
    {
        ...
        public async Task<bool> Handle(Guid subjectId, bool deleteWithRelated = false)
        {
            ...
            bool hasRelated = (grades?.Any() == true) ||
                              (attendances?.Any() == true) ||
                              (studentSubjects?.Any() == true);
            ...
            {
                if (hasRelated)
                    throw new InvalidOperationException("Нельзя удалить предмет, так как у него есть связанные оценки, посещаемость или связи со студентами.");
            }
